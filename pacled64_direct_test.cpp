...
// PacLed64 USB constants (from PacLed64.cpp)
...
public:
    PacLed64DirectTest() : m_context(nullptr), m_deviceHandle(nullptr)
    {
    ...
    bool Connect()
    {
        ...
        // Find and open PacLed64 device
        m_deviceHandle = libusb_open_device_with_vid_pid(m_context, PACLED64_VENDOR_ID, PACLED64_PRODUCT_ID);
        if (!m_deviceHandle)
        {
            std::cout << "ERROR: PacLed64 device not found (VID:0x" << std::hex << PACLED64_VENDOR_ID 
                      << " PID:0x" << PACLED64_PRODUCT_ID << std::dec << ")" << std::endl;
    ...
    bool SendLedUpdate()
    {
        ...
        // PacLed64 protocol from PacDriveSingleton:
        // libusb_control_transfer(usbDevice, 0x21, 9, 0x0200, 0, data, 2, 2000)
        // data[0] = port, data[1] = intensity
        ...
        for (int i = 0; i < 64; i++)
        {
            ...
            if (result < 0)
            {
                std::cout << "ERROR: Failed to send LED command for LED " << i 
    ...
    void RunTest()
    {
        ...
        while (ledIndex < 64)
        {
            std::cout << "\nTesting LED " << (ledIndex + 1) << " (0-based index: " 
                      << ledIndex << ")... " << std::flush;
            ...
            else if (input == 'r')
            {
                ...
                // Don't increment ledIndex, will repeat
            }
            else
            {
                // Default: next LED (Enter, 'n', or any other key)
                ledIndex++;
            }
        }
