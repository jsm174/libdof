...
template <typename MatrixElementType> void MatrixFlickerEffectBase<MatrixElementType>::DoFlicker()
{
   ...
   if (v > 0)
   {
      ...
      {
         std::uniform_int_distribution<int> indexDist(0, m_inactiveFlickerObjects.size() - 1);
      ...
      for (int i = m_activeFlickerObjects.size() - 1; i >= 0; i--)
      {
         FlickerObject& fo = m_activeFlickerObjects[i];

         int fv;
         int ageMs = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - fo.StartTimestamp).count();
