...
static struct cag_option options[]
   = { { .identifier = 'c', .access_letters = "c", .access_name = "config", .value_name = "VALUE", .description = "Config file (optional, default is no config file)" },
        { .identifier = 'a', .access_letters = "a", .access_name = "addr", .value_name = "VALUE", .description = "IP address or host name (optional, default is 'localhost')" },
        { .identifier = 'p', .access_letters = "p", .access_name = "port", .value_name = "VALUE", .description = "Port (optional, default is '6789')" },
        { .identifier = 'l', .access_letters = "l", .access_name = "logging", .value_name = NULL, .description = "Enable logging to stderr (optional, default is no logging)" },
        { .identifier = 'v',
           .access_letters = "v",
...
int main(int argc, char* argv[])
{
   ...
   while (cag_option_fetch(&cag_context))
   {
      ...
      if (identifier == 'c')
      {
         ...
         if (opt_verbose)
            DOF::Log(DOF_LogLevel_INFO, "Loaded config file");
      }
   ...
   sockpp::initialize();
   if (opt_verbose)
      DOF::Log(DOF_LogLevel_INFO, "Opening DOFServer, listining for TCP connections on %s:%d", pConfig->GetDOFServerAddr(), pConfig->GetDOFServerPort());
