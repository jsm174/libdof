...
PinscapeDevice::PinscapeDevice(hid_device* pDevice, const std::string& path, const std::string& name, unsigned short vendorID, unsigned short productID, unsigned short version)
{
   m_pDevice = pDevice;
   m_path = path;
   m_name = name;
...
bool PinscapeDevice::IsLedWizEmulator(int unitNum) { return (uint16_t)m_vendorID == 0xFAFA && m_productID == 0x00F1 + unitNum; }
